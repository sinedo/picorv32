...
-------------------------------
Four different Toolchains are used in this Task.

| Toolchain| Extensions | Description|
|----------|-----------|---------|
| RV32I | i extension | base 32 bit ISA |
| RC32IC  | i + c extension | some compressed (16bit) instructions to save code size |
| RV32IM  | i + m extension | multiply and divide instructions | 
| RV32IMC  | i + m + c extension |  |

It is possible to build all four toolchains from source in the `/opt` directory, see https://github.com/sevjaeg/picorv32.
For convinience we use a binary distribution that should work for all major operating systems.
...
to set the toolchains accordingly you simply pass the following argument to the compiler
